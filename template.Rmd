...
In this lab we will work with the **tidyverse**, **ggplot2**, **corrr** packages.
...
Hint: Plug ?cor() into your console to get information about how to run a correlation
...
4A. Answers can vary. The idea is that there should be no correlation for day, a positive correlation for month, and negative for year. Days shouldn't matter, months kinda matters, years really matters.
...
Stretch: Can you add in a `geom_smooth()` line to represent the correlation and `geom_point()` to add the mean number of crashes per day?

5A. This plot is slightly over engineered. They should answer that it's random and day has no relationship to car crash.
...
7A. Here there's much more interesting data to examine. If you needed to cut this down, I would only examine year. This is because year is the easiest to see that just the correlation doesn't provide evidence for everything.
```{r corr year}
...
### Part 2: Are specific days dangerous?
...
10A. Students should find there is no difference. The graph shows this as well.
```{r 420 fatalities}
...
Ass part of the article, Harper and Palayew (2019) could not find any signals that 4/20 is more dangerous than the average day. They did find other holidays, such as July 4th, were more dangerous.
...
11A. I wrote a function to figure this out. Overengineered? Yes. Is it cool? Also yes. This will be the next question's stretch goal. They should find that July 4th has higher car crashes than normal. 
...
12A. The Christmas one is representative of what students would most likely create. Christmas and New Years are not significantly different. The answer for the stretch is above in the July 4th data.
